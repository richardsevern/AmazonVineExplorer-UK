...
function addLeftSideButtons(forceClean) {
    ...
    const _setAllSeenDBBtn = createButton('Mark all as seen and Next','ave-btn-db-allseen', `left: 0; width: 240px; background-color: ${SETTINGS.BtnColorMarkAllAsSeen};`, () => {
    ...
    const _backToTopBtn = createButton('Top of Page','ave-btn-backtotop',  `width: 240px; background-color: ${SETTINGS.BtnColorBackToTop};`, () => {
        ...
        if (SETTINGS.EnableDesktopNotifikation && SETTINGS.DesktopNotifikationKeywords?.length > 0) {
            ...
            if (!_notifyed && _prodArrLength > oldCountOfNewItems){ 
                if (unixTimeStamp() - lastDesktopNotifikationTimestamp >= SETTINGS.DesktopNotifikationDelay) {
                    ...
                    desktopNotifikation(`Amazon Vine Explorer - ${AVE_VERSION}` , `Es wurden ${_prodArrLength} New Vine products found`);
                } 
            }
        }
    })
...
function desktopNotifikation(title, message, image = null, requireInteraction = null, onClick = () => {}) {
    ...
    const _vineLogoImp = 'https://raw.githubusercontent.com/richardsevern/AmazonVineExplorer-uk/main/vine_logo_important.png'
    const _defaultImage = 'https://raw.githubusercontent.com/richardsevern/AmazonVineExplorer-uk/main/vine_logo_notification_image.png'
...
function init(hasTiles) {
    ...
    _searchbarContainer.appendChild(createNavButton('ave-btn-list-new', 'New Items', 'ave-new-items-btn', SETTINGS.BtnColorNewProducts, () => {createNewSite(PAGETYPE.NEW_ITEMS);}, 'ave-new-items-btn-badge', '-'));
